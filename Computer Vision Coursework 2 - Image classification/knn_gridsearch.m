function [results] = knn_gridsearch(params, train_image_paths, test_image_paths, trG, teG, filename)
vals = values(params);
...
for i=1:length(all_combinations)
    ...
    norm_method = all_combinations(i, 7);
    k = all_combinations(i,6);
    k = k{1};
    distance_metric = all_combinations(i,2);
    ...
    feats_bin_size = all_combinations(i, 4);
    feats_bin_size = feats_bin_size{1};
    feats_step_size = all_combinations(i, 5);
    ...
    switch char(all_combinations(i, 3))
        case 'spatial'
    ...
    labs = kNN(k, distance_metric, p, train_features, test_features, trG);
