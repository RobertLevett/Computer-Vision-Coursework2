function results = ada_gridsearch(params, train_image_paths, test_image_paths, trG, teG, filename)
vals = values(params);
...
for i=1:length(all_combinations)
    ...
    learners = all_combinations(i, 5);
    learners = learners{1};
    
    lr = all_combinations(i, 6);
    lr = lr{1};
    
    ens = fitensemble(train_features,trG,'AdaBoostM2',learners,'Tree', 'LearnRate', lr);
    
    acc = 1 - loss(ens, test_features, teG)
    
    all_res{i, 1} = all_combinations(i, :);
    all_res{i, 2} = acc;
